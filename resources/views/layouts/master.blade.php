...
<header class="relative flex items-center h-24 py-4 bg-white" role="banner">
    <div class="container flex items-center px-4 mx-auto lg:px-8">
        <div class="flex items-center">
            <!-- Desktop Logo -->
            <a href="/" title="{{ config('app.name') }} home" class="items-center hidden md:inline-flex">
                {!! file_get_contents(public_path('/img/logo.svg')) !!}
            </a>
            <!-- Mobile Logo -->
            <a href="/" title="{{ config('app.name') }} home" class="inline-flex items-center md:hidden">
                {!! file_get_contents(public_path('/img/logo-small.svg')) !!}
            </a>
        </div>
